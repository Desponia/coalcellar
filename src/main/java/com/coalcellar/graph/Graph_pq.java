...
class Graph_pq {
    ...
    public Graph_pq(int V) {
        ...
        dist = new int[V];
        visited = new HashSet<>();
        pqueue = new PriorityQueue<>(V, Comparator.comparingInt(o -> o.cost));
    }
...
class Main {
    public static void main(String arg[]) {
        ...
        Graph_pq dpq = new Graph_pq(vertex);
        dpq.algo_dijkstra(adj_list, start);
        ...
        for (int i = 0; i < dpq.dist.length; i++)
            System.out.println(start + " \t\t " + i + " \t\t " + dpq.dist[i]);
    }
}
